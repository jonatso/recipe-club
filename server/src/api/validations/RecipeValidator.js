...
const validateInput = (body) => {
	for (let i = 0; i < body.ingredients.length; i++) {
		if (!body.ingredients[i].name) {
			throw new Error(`name is missing from ingredient ${i + 1}`);
		}
		if (!body.ingredients[i].quantity) {
	...
	// We could add more validation of the types, but it should not be necessary as they convert nicely into strings regardless
};
