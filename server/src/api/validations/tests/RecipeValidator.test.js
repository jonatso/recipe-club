...
test("ingredient without quantity", () => {
	...
	expect(test).toThrow(
		new Error(`quantity is missing from ingredient ${0 + 1}`)
	);
});
...
test("quantity not an integer", () => {
	...
	expect(test).toThrowError(
		new Error(`quantity in ingredient ${0 + 1} is not an integer`)
	);
});
...
test("ingredient without unit", () => {
	...
	expect(test).toThrowError(
		new Error(`unit is missing from ingredient ${0 + 1}`)
	);
});
...
test("picture file is invalid", () => {
	...
	expect(test).toThrowError(
		new Error("picture is must be either .jpg or .png")
	);
});
