...
test("Given no recipes exist, getRecipes statusCode is 404", async () => {
   expect.assertions(1);
   const { statusCode } = await supertest(app).get(`/recipes/`);
...
test("Using wrong inputfields (noncoherent to database columns), createRecipe the statusCode is 400", async () => {
   expect.assertions(1);
   const { statusCode } = await supertest(app).post(`/recipes/create`).send(badRecipePayload);
...
test("Using correct inputfields (coherent to database columns), createRecipe the statusCode is 201 and object is created", async () => {
   expect.assertions(2);
   const { statusCode } = await supertest(app).post(`/recipes/create`).send(recipePayload);
...
test("If recipeId is not integer, updateRecipe statusCode is 400", async () => {
   expect.assertions(1);
   const recipeId = "blah";
   const { statusCode } = await supertest(app).put(`/recipes/update/${recipeId}`).send(recipePayloadUpdate);
   expect(statusCode).toBe(400);
});
...
test("Using correct inputfields (noncoherent to database columns), updateRecipe the statusCode is 200 and the object is updated", async () => {
   expect.assertions(2);
   const recipe = await Recipe.create(recipePayload);
   const { statusCode } = await supertest(app).put(`/recipes/update/${recipe.id}`).send(recipePayloadUpdate);
...
test("Using wrong inputfields (noncoherent to database columns), updateRecipe the statusCode is 400", async () => {
   expect.assertions(1);
   const recipe = await Recipe.create(recipePayload);
   const { statusCode } = await supertest(app).put(`/recipes/update/${recipe.id}`).send(badRecipePayload);
   expect(statusCode).toBe(400);
});
...
test("If recipeId is not integer, deleteRecipe statusCode is 400", async () => {
   expect.assertions(1);
   const recipeId = "blah";
   const { statusCode } = await supertest(app).delete(`/recipes/delete/${recipeId}`);
   expect(statusCode).toBe(400);
});
...
test("If recipeId is integer, deleteRecipe statusCode is 200 and object is deleted", async () => {
   expect.assertions(2);
   const recipe = await Recipe.create(recipePayload);
   const { statusCode } = await supertest(app).delete(`/recipes/delete/${recipe.id}`);
