      ...
      <TabPanels>
         ...
         <SimpleGrid columns={[1, 2, 3]} spacing={3}>
            {userRecipes.isSuccess ? (
               <>
                  {userRecipes.data[0] ? (
                     userRecipes.data.map((recipe) => <RecipeCard key={recipe.name + recipe.id} recipe={recipe} />)
                  ) : (
         ...
         <SimpleGrid columns={[1, 2, 3]} spacing={3}>
            {savedRecipes.isSuccess ? (
               <>
                  {savedRecipes.data[0] ? (
                     savedRecipes.data.map((recipe) => <RecipeCard key={recipe.name + recipe.id} recipe={recipe} />)
                  ) : (
                     <span>There are no recipes...</span>
                  )}
               </>
